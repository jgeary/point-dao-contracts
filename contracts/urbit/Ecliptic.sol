    ...
    IERC721Metadata
{
    ...
    //        ERC721Holder(0).onERC721Received.selector`
    ...
    function safeTransferFrom(
        ...
        //
        if (_to.isContract()) {
            bytes4 retval = ERC721Holder(_to).onERC721Received(
                msg.sender,
                _from,
                _tokenId,
                _data
        ...
        returns (string memory _tokenURI)
    {
        _tokenURI = string(
            abi.encodePacked(
                "https://azimuth.network/erc721/",
                48 + ((_tokenId / 1000000000) % 10),
                48 + ((_tokenId / 100000000) % 10),
                48 + ((_tokenId / 10000000) % 10),
                48 + ((_tokenId / 1000000) % 10),
                48 + ((_tokenId / 100000) % 10),
                48 + ((_tokenId / 10000) % 10),
                48 + ((_tokenId / 1000) % 10),
                48 + ((_tokenId / 100) % 10),
                48 + ((_tokenId / 10) % 10),
                48 + ((_tokenId / 1) % 10),
    ...
    function spawn(uint32 _point, address _target) external {
        ...
        //
        require(azimuth.isOwner(_point, address(0)));
        ...
        if (msg.sender == _target) {
            doSpawn(_point, _target, true, address(0));
        }
        //
    ...
    function doSpawn(
        ...
        if (_direct) {
            ...
            emit Transfer(address(0), _target, uint256(_point));
        }
        //
        ...
        //
            ...
            emit Transfer(address(0), _holder, uint256(_point));
    ...
    function transferPoint(
        ...
        if (!azimuth.isOwner(_point, _target)) {
            ...
            //
            azimuth.setTransferProxy(_point, address(0));
        ...
        if (depositAddress == _target) {
            ...
            azimuth.setManagementProxy(_point, address(0));
            azimuth.setVotingProxy(_point, address(0));
            azimuth.setTransferProxy(_point, address(0));
            azimuth.setSpawnProxy(_point, address(0));
        ...
        activePointVoter(_galaxy)
    {
        ...
        polls.startUpgradePoll(address(_proposal));
    }
    ...
    function castUpgradeVote(
        ...
        //
        bool majority = polls.castUpgradeVote(
            _galaxy,
            address(_proposal),
            _vote
        );
    ...
    function updateUpgradePoll(EclipticBase _proposal) external {
        ...
        //
        bool majority = polls.updateUpgradePoll(address(_proposal));
    ...
    function upgradeTreasury(address _treasuryImpl) external {
        ...
        require(
            keccak256(abi.encodePacked(_treasuryImpl)) == treasuryUpgradeHash
        );
    ...
    function createGalaxy(uint8 _galaxy, address _target) external onlyOwner {
        ...
        //
        require(azimuth.isOwner(_galaxy, address(0)) && address(0) != _target);
        ...
        //
        if (msg.sender == _target) {
            doSpawn(_galaxy, _target, true, address(0));
        }
        //
        //  when sending to a "foreign" address, enforce a withdraw pattern,
    ...
    function setDnsDomains(
        ...
        string calldata _secondary,
        string calldata _tertiary
    ) external onlyOwner {
